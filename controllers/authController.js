...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Generete the random rest token
  ...
  try {
    ...
    res.status(200).json({
      ...
      token: resteToken // 🖕Don't send token
    });
  } catch (err) {
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not expited, and there is user, set the new password
  if (!user) {
  ...
  // 3) Update changedPasswordAt property for user using presave hock
